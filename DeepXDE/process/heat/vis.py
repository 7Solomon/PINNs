...
def visualize_divergence(model, subtype, inverse_scale=None):
    from utils.COMSOL import load_COMSOL_file_data, analyze_COMSOL_file_data
    if subtype == 'steady':
        from vis import vis_steady_diffrence
        file_data = load_COMSOL_file_data(os.path.join('BASELINE', 'heat', '2d_steady.txt'))
        domain, temp = analyze_COMSOL_file_data(file_data)
        ...
        vis_time_diffrence(model, file_data, inverse_scale=inverse_scale)