...
def get_2d_domain(domain_variabeles: Domain, scale_x, scale_y):
    ...
    scaled_points = np.vstack((scaled_X.flatten(), scaled_Y.flatten())).T  
...
def get_2d_time_domain(domain_variabeles:Domain, scale_x, scale_y, scale_t):
    ...
    scaled_X, scaled_Y, scaled_T = np.meshgrid(scaled_x, scaled_y, scaled_t) 
    ...
    return {'normal':[points, X, Y, T], 'scaled': [scaled_points, scaled_X, scaled_Y, scaled_T]}
