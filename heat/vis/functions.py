import sys
from vis.basic import get_T_pred_grid, get_steady_T_pred_grid, plot_loss, vis_plate
...
def visualize_field(model, domain):
    ...
    try:
        ...
        if 't' in domain.header.keys():
            ...
            data = get_T_pred_grid(model, device, x, y, t, n_grid_points=100)
            vis_plate(data)
            return
        data = get_steady_T_pred_grid(model, device, x, y, n_grid_points=100)
        vis_plate(data)
